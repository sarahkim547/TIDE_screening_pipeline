...
Input:1. Input file
      ...
      7. Max (use best N guides)
Edited by Peter Du (Mar 2018)
'''
import pandas as pd
import numpy as np
...
def get_parser():
    ...
    required.add_argument('--input-file',
        ...
        required=True)
    required.add_argument("--output-dir", 
	type=str,
    ...
    parser.add_argument('--max',
        type=int,
        default=-1,
        help='Only use the best N guides for every gene; default = all')
    return parser
...
if __name__ == '__main__':
    ...
    # get columns to be analyzed and rename them
    ...
    # read chip and rename column names
    ...
    # output directory
    #out_dir = os.path.dirname(inputfile) + '/STARS_output_' + str(datetime.now().strftime("%y-%m-%d-%H-%M-%S"))
    out_dir= args.output_dir 
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    print '\nOutput will be written to ' + out_dir

    # counter for number of columns processed
    counter = 1
    for ci,c in enumerate(cols_iter):
        outputfile = out_dir + '/' + c+'_STARSOutput_'+direction+'_'+str(thr)
        if max_pert > 0:
        ...
        # create dictionary of spacers with linked rank
        sp_rank = {}
        ...
        # ge is deduped list of gene symbols
        ...
        with open(outputfile,'w') as o:
            ...
            print 'Analyzing ... ' + str(counter) + '/' + str(len(input_df.columns) - 1)
            counter += 1
            for g in ge:
                #print i
                i=i+1
                ...
                if first_pert == 'N' or first_pert == 'n':
                    ...
                    sys.exit(1)
                # indices = ranks of the genes above threshold
                indices = [r for r in rank if r <= thr_rank]

                # remove until <= max_pert
        ...
        sp_stars = sp_stars.sort_values(['STARS Score'], ascending=False)
